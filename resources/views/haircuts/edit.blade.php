...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black dark:text-white"> Editar Corte </h2>
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="font-medium text-primary">Editar corte</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
    <div class="flex flex-col gap-9 sm:grid-cols-2">
        <div class="flex flex-col gap-9">
            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <form action="{{ route('haircuts.update', $haircut->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="p-6.5">
                        <div class="mb-4.5">
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent dark:bg-form-input">
                                <select id="employee_id" name="employee_id" required
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ $haircut->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4.5">
                            ...
                            <div class="relative">
                              <input type="date" id="date" name="date" value="{{ $haircut->date }}" required
                        ...
                        <div class="mb-4.5">
                            ...
                            <input type="number" step="0.01" id="price" name="price" value="{{ $haircut->price }}" required placeholder="Ingresa el precio"
                        ...
                        <div class="mb-4.5">
                            ...
                            <input type="text" id="feature" name="feature" value="{{ $haircut->feature }}" placeholder="Ingresa la característica"
                                class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                        </div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent dark:bg-form-input">
                                <select id="products" name="products[]" multiple
                                    ...
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ in_array($product->id, $haircut->products->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $product->name }}</option>
                                    @endforeach
                                </select>
                                <span class="absolute right-4 top-1/2 z-30 -translate-y-1/2">
                                    <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24"
                        ...
                        <div class="mb-4.5">
                            ...
                            <input type="number" step="0.01" id="price2" name="price2" value="{{ $haircut->price2 }}" placeholder="Ingresa el precio2"
                                class="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                        </div>
